    ...
    private Boolean isSelected = false;  //注意实体类中，还可以定义这种与数据库无对应的列，并且设置一个默认值的写法


以后封装接口返回给前台，比如客户标签，及标签明细，直接将系统中所有的客户标签及标签明细数据返回，并且带上是否选中的标识传给前台用于客户新建，客户修改时数据的回显
...
比如有多表，不仅仅是需要返回客户的数据，还需要顺带user的数据的返回结果集，返回java.util.HashMap没有问题(注意此时使用resultType而不再使用resultMap去上面找是否有定义的defaultMap了)
如下：
